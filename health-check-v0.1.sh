...
if [[ "$DISCONNECTED_PEERS" -gt 0 ]]; then
...
# ==========================================================
VOL_STATUS=$(gluster volume info "$VOL_NAME" 2>/dev/null | grep "^Status:" | awk '{print $2}')
if [[ "$VOL_STATUS" != "Started" ]]; then
    log_issue "CRITICAL" "Volume $VOL_NAME is $VOL_STATUS (expected: Started)"
fi

# ==========================================================
# CHECK 4: Brick status (Online/Offline)
# ==========================================================
...
if [[ "$HEAL_ENTRIES" -gt 0 ]]; then
    ...
    # Trigger automatic heal
    log_message "INFO" "Triggering automatic heal..."
    gluster volume heal "$VOL_NAME" full >/dev/null 2>&1
    log_message "INFO" "Heal command sent. Check status: gluster volume heal $VOL_NAME info"
fi

# ==========================================================
# CHECK 6: Split-brain (CRITICAL)
# ==========================================================
...
if [[ "$ISSUES" -eq 0 ]]; then
    ...
    log_message "INFO" "Peers: $CONNECTED_PEERS/$TOTAL_PEERS connected | Bricks: $TOTAL_BRICKS online | Disk: ${DISK_USAGE:-N/A}% used | Heal pending: $HEAL_ENTRIES"
    exit 0
else
